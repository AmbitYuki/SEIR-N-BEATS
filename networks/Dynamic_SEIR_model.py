...
class Train_Dynamic_SEIR:
    ...
    def _update(self):
        ...
        E =  2.71828182846
        # E = 2.71
        alpha_eta  = 0.0000000008; # learning rate
        b_eta = 0.00000008; # learning rate
        c_eta = 0.00000008; # learning rate
        # alpha_eta = 0.000000001;  # learning rate
        # b_eta = 0.000000001;  # learning rate
        # c_eta = 0.000000001;  # learning rate
        alpha_temp=0.0;
        c_temp=0.0;
        b_temp =0.0;
        ...
        for e in range(self.epoch):
            ...
            if e == (self.epoch - 1):
                ...
                'Estimated_Susceptible':self.S_pre, 'Estimated_Exposed': self.E_pre, 'Estimated_Infected':self.I_pre, 
                                                              'Estimated_Resistant':self.R_pre,'est_beta':self.betalist},
    ...
    def plot_fitted_beta_R0(self, real_obs: pandas.core.frame.DataFrame):
        ...
        plt.ylabel('Rate')
        plt.title('Fitted dynamic contact rate over time', fontsize = 20)
    ...
    def plot_fitted_result(self, real_obs: pandas.core.frame.DataFrame):
        ...
        plt.legend(['Estimated Infected','Real Infected', 'Estimated_Exposed', 'Real Exposed'], prop={'size': 10}, bbox_to_anchor=(0.5, 1.02),
    ...
    def run(self, death_rate):
        ...
        self.results = pd.DataFrame.from_dict({'Time':list(range(len(Susceptible))),
            'Susceptible':Susceptible, 'Exposed': Exposed, 'Infected':Infected, 'Resistant':Resistant,
                                               'Death':Death, 'Heal': Heal,'est':rateSI},
            orient='index').transpose()
        self.modelRun = True
        return self.results
