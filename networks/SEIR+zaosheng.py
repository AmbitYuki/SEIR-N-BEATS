...
## Load data
df = pd.read_csv("D:/covid_my/SEIR_total1/covid1.csv")
...
China_df = df[df['date'] < datetime.datetime(2020, 2, 16)]
# China_total = get_China_total(China_df)
# print(China_total)
# China_total.to_csv('date')
Dynamic_SEIR1 = Train_Dynamic_SEIR(epoch = 10000, data = China_df,
                 population = 1400000000, rateAl = 1/7, rateIR=1/14, c = 1, b = -10, alpha = 0.08)

estimation_df = Dynamic_SEIR1.train()#train
print(estimation_df)
# 2.3
test = get_China_total(df[df['date'] < datetime.datetime(2020, 2, 16)])
...
err = err.reset_index()
China_total = estimation_df.reset_index()
err.columns = ['index','err']
print(err)
err_SEIR = pd.concat([China_total, err['err']],axis=1)
# est_beta
print(err_SEIR)
err_SEIR.to_csv("D:/下载/n-beats-master新/n-beats-master/examples/data/err_SEIR_zaosheng1.csv")
...
# result.to_csv('D:/covid_my/SEIR_total1/pred1_zaosheng1.csv')
