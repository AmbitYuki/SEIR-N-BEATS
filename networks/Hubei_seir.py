...
## Load data
df = pd.read_csv("D:/下载/n-beats-master新/n-beats-master/examples/data/DXYArea.csv")
...
Hubei_total = get_province_df(China_df,'Hubei Province')
print(Hubei_total)
# China_total.to_csv('date')
Dynamic_SEIR1 = Train_Dynamic_SEIR(epoch = 10000, data = Hubei_total,
                 population = 58500000, rateAl = 1/7, rateIR=1/14, c = 1, b = -10, alpha = 0.08)
...
print(err)
err_SEIR = pd.concat([Hubei_total, err['err'],est_beta],axis=1)
# est_beta
print(err_SEIR)
...
Dynamic_SEIR1.plot_fitted_beta_R0(Hubei_total)

Dynamic_SEIR1.plot_fitted_result(Hubei_total)
...
I0 = list(Hubei_total['I'])[-1]
R0 = list(Hubei_total['R'])[-1]
# suppose the total number of individuals within incubation period is 4 time of current suscepted cases
E0 = list(Hubei_total['E'])[-1] *4
S0 = population - I0 - E0 - R0

seir_new = dynamic_SEIR(eons=7, Susceptible=S0, Exposed = E0,
                    Infected=I0, Resistant=R0, rateIR=1/14,
                    rateAl = 1/7, alpha = est_alpha, c = est_c, b = est_b, past_days = Hubei_total['Days'].max())
result = seir_new.run(death_rate = 0.02) # assume death rate is 2%
seir_new.plot_noSuscep('Dynamic SEIR for China total', 'population', 'Date', starting_point = Hubei_total['date'].max())
print(result)
"""
Calculate MAPE test score using SEIR model result
"""
China_df1 = SIR_data[SIR_data['date'] >=datetime.datetime(2020, 2, 16)]
Hubei_test= get_province_df(China_df1,'Hubei Province')
Hubei_test.to_csv('D:/covid_my/SEIR_total1/Hubei_test.csv')
# test = SIR_data[SIR_data['date'] >= datetime.datetime(2020, 2, 16)])
