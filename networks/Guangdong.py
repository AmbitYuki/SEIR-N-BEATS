...
## Load data
df = pd.read_csv("E:/下载/n-beats-master新/n-beats-master/examples/data/Guangdong2.csv")
...
Shanghai_df = SIR_data[SIR_data['date'] < datetime.datetime(2020,3, 25)]
# China_total = get_China_total(SIR_data)
# print(China_total)
# China_total.to_csv('date')
Dynamic_SEIR1 = Train_Dynamic_SEIR(epoch = 10000, data = Shanghai_df ,
                 population = 126012510, rateAl = 1/7, rateIR=1/14, c = 1, b = -10, alpha =0.8)

estimation_df = Dynamic_SEIR1.train()#train
print(estimation_df)
# 2.3
test = SIR_data[SIR_data['date'] < datetime.datetime(2020, 3, 25)]
...
err_SEIR.to_csv("E:/下载/n-beats-master新/n-beats-master/examples/data/Guangdong3_SEIR.csv")
# est_beta
print(err_SEIR)
...
result.to_csv('E:/下载/n-beats-master新/n-beats-master/examples/data/pre_Guangdong3.csv')
