...
## Load data
df = pd.read_csv("D:/下载/n-beats-master新/n-beats-master/examples/data/shanghai_101.csv")
...
Shanghai_df = SIR_data[SIR_data['date'] < datetime.datetime(2020,4, 25)]
# China_total = get_China_total(SIR_data)
# print(China_total)
# China_total.to_csv('date')
Dynamic_SEIR1 = Train_Dynamic_SEIR(epoch = 10000, data = Shanghai_df ,
                 population = 24870895, rateAl = 1/7, rateIR=1/14, c = 1, b = -10, alpha =0.8)

estimation_df = Dynamic_SEIR1.train()#train
print(estimation_df)
# 2.3
test = SIR_data[SIR_data['date'] < datetime.datetime(2020, 4, 25)]
...
# est_beta
print(err_SEIR)
err_SEIR.to_csv("D:/下载/n-beats-master新/n-beats-master/examples/data/err_SEIR_shanghai101.csv")
...
seir_new = dynamic_SEIR(eons=7, Susceptible=S0, Exposed = E0,
                    ...
                    rateAl = 1/7, alpha = est_alpha, c = est_c, b = est_b, past_days = Shanghai_df['Days'].max())
...
result.to_csv('D:/下载/n-beats-master新/n-beats-master/examples/data/shanghai101_pre.csv')
