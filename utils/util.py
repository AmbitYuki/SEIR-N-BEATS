...
def get_batch(batch_size, i_batch,  model, seq_lengths, seq_times, seq_types, seq_acts,  rnn = True):#添加部分 seq_acts,
    start_pos = i_batch
    end_pos = i_batch + batch_size
    batch_seq_lengths = seq_lengths[start_pos:end_pos]
    max_seq_length = batch_seq_lengths[0]#对于每个i_batch 选择最长的序列 不同的i_batch max_seq_length不同，因为每个序列长度不同，seq_length已经按从大到小排好序
    # max_seq_length = 700
    batch_seq_times = seq_times[start_pos:end_pos, :max_seq_length + 1]#0维是batch 1维是max_seq_length
    batch_seq_types = seq_types[start_pos:end_pos, :max_seq_length + 1]
    batch_seq_acts = seq_acts[start_pos:end_pos, :max_seq_length + 1] #修改部分
    # Inter-event time intervals
    batch_dt = batch_seq_times[:, 1:] - batch_seq_times[:, :-1]

    batch_onehot = one_hot_embedding(batch_seq_types, model.input_size) #返回一个为数是（序列长度，事件类型+1）
    ...
    if rnn:
        ...
        packed_acts = nn.utils.rnn.pack_padded_sequence(batch_seq_acts, batch_seq_lengths, batch_first=True)#添加
        max_pack_batch_size = packed_dt.batch_sizes[0]
    else:
        # self-attention
        packed_dt,packed_types, packed_acts,max_pack_batch_size = None, None, None,0
    return batch_onehot, batch_seq_times,  batch_dt, batch_seq_types, \
           max_pack_batch_size, packed_dt,packed_acts, packed_types, batch_seq_acts, batch_seq_lengths#添加packed_types
