...
def process_loaded_sequences(loaded_hawkes_data: dict, process_dim: int) -> Tuple[Tensor, Tensor, Tensor]:
    ...
    even_action_list = loaded_hawkes_data['acts'] #修改部分
    ...
    even_action_list = [torch.from_numpy(e) for e in even_action_list] #修改部分
    tmax = 0
    ...
    seq_times = torch.cat((torch.zeros_like(seq_times[:, :1]), seq_times), dim=1) # add 0 to the sequence beginning cat用来拼接
    ...
    seq_acts = nn.utils.rnn.pad_sequence(even_action_list, batch_first=True, padding_value=process_dim)#修改部分
    seq_acts = torch.cat(
        (process_dim * torch.ones_like(seq_acts[:, :1]), seq_acts), dim=1).long()#修改部分
    return seq_times, seq_types, seq_lengths, tmax, seq_acts
...
def one_hot_embedding(labels: Tensor, num_classes: int) -> torch.Tensor:
    ...
    y = torch.eye(num_classes).to(device)#这个函数主要是为了生成对角线全1，其余部分全0的二维数组
    return y[labels]