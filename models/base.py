...
class SeqGenerator:
    ...
    def plot_events_and_intensity(self, model_name: str = None, debug=False):
        ...
        fig, ax = plt.subplots(1, 1, sharex='all', dpi=100,
                               figsize=(9, 4.5))#dpi设置图像每英寸的点数 sharex=all表示所有子图共享x和y轴
        ax: plt.Axes#绘图总是在坐标系上 表示区域
        inpt_size = self.process_dim
        ax.set_xlabel('Time $t$ (s)')
        intens_hist = np.stack(self.intens_hist)[:, 0]#第0列堆叠
...
def predict_from_hidden(model, h_t_vals, dt_vals, next_dt, next_type, plot, hmax: float = 40.,
    ...
    intens_t_vals_sum = intens_t_vals.sum(dim=1)#列归一， 横向压缩
    integral_ = torch.cumsum(timestep * intens_t_vals_sum, dim=0)#第1行不动，将第1行累加到其他行
    ...
    estimate_type_prob = (timestep * 0.5 * (prob_type[1:] + prob_type[:-1])).sum(dim=0)#结合公式来看 差不多懂了
    if print_info:
    ...
    if plot:
        ...
        return (estimate_dt, next_dt, error_dt, next_type, estimate_type), fig #此部分已看 差不多懂了
